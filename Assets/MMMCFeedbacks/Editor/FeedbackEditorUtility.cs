...
namespace MMMCFeedbacks.Editor
{
    public static class FeedbackEditorUtility
    {
        internal static bool Foldout(Rect rect,string label, Color tagColor,bool isActive,ref bool isMenu ,ref bool isExpanded)
        {
            ...
            // labelのstringの（）とその中身だけ薄く表示してその他はBoldにする
            var index = label.IndexOf('(');
            if (index != -1)
            {
                var labelString = label.Substring(0, index);
                var labelParameter = label.Substring(index);
                EditorGUI.LabelField(labelRect, labelString,EditorStyles.boldLabel);
                labelRect.xMin += EditorStyles.boldLabel.CalcSize(new GUIContent(labelString)).x;
                EditorGUI.LabelField(labelRect, labelParameter, EditorStyling.grayBoldLabel);
            }
            else
            {
